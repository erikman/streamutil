...
class ReadableWrapper extends stream.Readable {
  ...
  _read() {
    try {
      ...
      console.error('ReadableWrapper', err.stack);
      this.emit('error', err);
    }
  }
}
...
// Call 'map' function on each element (possibly returning promises).
export class MapStream extends stream.Transform {
  ...
  _transform(data, encoding, done) {
    try {
      var mapResult = this.mapper.call(null, data);
      if (mapResult && _.isObject(mapResult) && _.isFunction(mapResult.then)) {
...
export class StreamStream extends stream.Readable {
  ...
  _read() {
    while (this.streams.length > 0) {
      ...
      let obj = this.streams[0].read();
      if (obj === null) {
        // End of stream
        this.streams.shift();
        this.isNewStream = true;
      }
      else if (!this.push(obj)) {
  ...
  function destroyStreams() {
    destroyCallbacks.forEach(destroyCallback => {
      try {
        ...
        console.error('Error while invoking destroy callback', err.stack);
        errorHandler(err);
      }
    });
  }
